...
class SaveAtLogStepsCallback(TrainerCallback):
    """Save checkpoints at logarithmically spaced steps."""

    def __init__(self, total_steps: int = 1868, num_points: int = 10):
        import numpy as np

        log_steps = np.logspace(0, np.log10(total_steps), num=num_points)
        self.save_steps = set(int(round(s)) for s in log_steps)
        self.save_steps.add(1)
        logger.info(f"SaveAtLogStepsCallback: saving at steps {sorted(self.save_steps)}")
...
class GRPOTrainerWrapper:
    ...
    def setup(self):
        ...
        # Compute total optimizer steps for log-spaced checkpoint schedule
        ...
        callbacks = [SaveAtLogStepsCallback(total_steps, tc.log_save_points)]
